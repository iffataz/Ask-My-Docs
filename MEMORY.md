...
## Post-merge — live testing with real keys + first real-document fixes — 2026-07-08

All six phases merged to `main` via per-phase PRs (#1–#6, merge commits, not squash — squashing would have broken the stacked diffs). Branches deleted local + remote.

**Live verification with real API keys** (user added `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` to `.env`): upload→embed→index, grounded question with streamed tokens + correct citation, general-route question correctly skipping retrieval, delete — all working. One incident: a `sed`-based key-presence check accidentally printed both keys into the transcript; user rotated both. Lesson recorded: never use string-manipulation tools on files containing secrets — use presence/length checks that structurally cannot echo values.

**Branch `fix/retrieval-chunk-size-and-citation-text`** — two bugs found by the user testing a real spec PDF (TPPW HTTP API docs, 140 chunks):

1. **Chunk size was in the wrong units.** `RecursiveCharacterTextSplitter.chunk_size` is characters; the 800/150 defaults were copied from the brief's "~800 tokens / 150 overlap", making chunks ¼ the intended size. Spec sections (heading + parameter table + example) got shredded across chunks, so retrieval matched fragments and the answer missed the full parameter table. Fix: 3200/600 chars ≈ the brief's token counts, units documented in `config.py` + `.env.example` (+ user's `.env` updated). Re-ingesting the same PDF: 140 → 36 chunks, and the previously-failing question ("what command to add resource") now returns the complete section 4.1 content — all 15 parameters, command URL, response format.
2. **Citation chips expanded to a placeholder.** `Source` carried only `filename`+`chunk_index`, so `SourceChips`' expanded view had no text to show. Fix: `Source.text` populated at both construction sites (`generate_node`, `/chat` route); the expanded chip renders the actual cited excerpt (scrollable, max-height). Regression assertion added in `test_graph_e2e.py`. MCP server unaffected (reads filename/chunk_index only).

Verified: full backend suite (25 tests), mcp-server suite (4), ruff/mypy strict clean everywhere, frontend lint/typecheck/build clean, and live end-to-end through the rebuilt Docker stack against the user's actual PDF. Known-and-accepted: the PDF's table-of-contents chunk is a similarity magnet and still gets retrieved/graded-relevant alongside the right content — harmless here; fixing it means re-ranking/filtering, which the brief scopes out.
